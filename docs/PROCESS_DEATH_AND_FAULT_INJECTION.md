...
ACK en checkpoints verificables. Desde el 24 de septiembre de 2026 la app no tiene transporte
remoto: envío y ACK se ejercitan contra el código de outbox compartido y transportes de prueba. El
comando canónico es:
...
También se puede ejecutar una capa aislada con `jvm` o `android`; la capa `firebase` se retiró
junto con la variante cloud. La capa `android` exige un emulador: el runner rechaza un serial que no
sea `emulator-*`, porque `connectedAndroidTest` desinstala la app al terminar y en la tablet del
negocio borraría sus datos. El directorio de evidencia debe ser nuevo: el runner no sobrescribe
resultados anteriores y siempre genera `gates.tsv` más un log por gate.
...
| Efecto remoto, antes del ACK | Clave idempotente durable | Se reenvía la misma clave; dos entregas producen un solo efecto remoto | `OutboxReplayFaultInjectionTest` |
...
es un `SIGKILL` real en mitad de una instrucción de CameraX o de un transporte remoto; esas ventanas se
...
| Red cortada | JVM: conectividad compartida del transporte | La fila vuelve a `PENDING`, conserva clave y completa tras reconexión |
| HTTP 429 | JVM: puerto del drenador con resultado transitorio inyectado | Backoff durable, contador creciente y misma clave |
| HTTP 5xx / timeout | JVM: puerto del drenador con resultado inyectado | Nunca se marca éxito; reintenta y al límite queda `FAILED` manual |
...
| Duplicado / ACK perdido | JVM remoto idempotente | Repetir la misma clave devuelve el mismo recibo y crea un solo efecto |
...
| Dos dispositivos | JVM: dos outboxes independientes contra un transporte compartido | Ambos clientes convergen y el transporte de prueba materializa una sola identidad |
...
- `android` usa Room, SQLite, archivos privados, Android Keystore y WorkManager en un emulador,
  nunca en la tablet del negocio. ENOSPC se inyecta como excepción real del API, pero no ocupa todo
  el disco del host.
