...
class DebtDeleteScreenTest {
    ...
    fun confirmationShowsExactDebtStockAndRefundAndRequiresExplicitCancelOrConfirm() {
        ...
        composeRule.onNodeWithText(context.getString(R.string.debt_delete_debtor, target.debtorName)).performScrollTo().assertIsDisplayed()
        composeRule.onNodeWithText(context.getString(R.string.debt_delete_identity, target.debtId.value)).performScrollTo().assertIsDisplayed()
        composeRule.onNodeWithText(context.getString(R.string.debt_delete_date, target.createdAt.formatForDisplay())).performScrollTo().assertIsDisplayed()
        composeRule.onNodeWithText(context.getString(R.string.debt_delete_original, target.originalAmount.formatForDisplay())).performScrollTo().assertIsDisplayed()
        composeRule.onNodeWithText(context.getString(R.string.debt_delete_balance, target.balance.formatForDisplay())).performScrollTo().assertIsDisplayed()
        composeRule.onNodeWithText(context.getString(R.string.debt_delete_line, "Arroz", "2", "NIU", "Principal")).performScrollTo().assertIsDisplayed()
        composeRule.onNodeWithText(context.getString(R.string.debt_delete_refund, preview.refundAmount.formatForDisplay())).performScrollTo().assertIsDisplayed()
        composeRule.onNodeWithText(context.getString(R.string.debt_delete_history)).performScrollTo().assertIsDisplayed()
    ...
    fun previewLoadingAndCommitBlockConfirmationAndStaleOffersReviewInsteadOfMutation() {
        ...
        composeRule.setContent { FacturaStockTheme { DebtDetailScreen(state, actions::add) } }
        dialogButton(R.string.debt_delete_loading).assertIsNotEnabled()
        dialogButton(R.string.debt_delete_cancel).assertIsEnabled()
        composeRule.runOnIdle { state = dialogState().copy(isDeletingDebt = true) }
        dialogButton(R.string.debt_delete_saving).assertIsNotEnabled()
        dialogButton(R.string.debt_delete_cancel).assertIsNotEnabled()
        ...
        composeRule
            ...
            .performScrollTo()
            .assertTextContains(context.getString(R.string.debt_delete_error_stale))
        dialogButton(R.string.debt_delete_review).assertIsEnabled().performClick()
    ...
    fun doubleFontSizeKeepsBothActionsVisibleWhileImpactScrolls() {
        ...
        dialogButton(R.string.debt_delete_confirm).assertIsDisplayed().assertIsEnabled()
        dialogButton(R.string.debt_delete_cancel).assertIsDisplayed().assertIsEnabled()
        composeRule
            .onNodeWithText(context.getString(R.string.debt_delete_refund, money("5").formatForDisplay()))
            .performScrollTo()
            .assertIsDisplayed()
        dialogButton(R.string.debt_delete_confirm).assertIsDisplayed()
        dialogButton(R.string.debt_delete_cancel).assertIsDisplayed()
    }

    private fun dialogButton(label: Int) =
        composeRule.onNode(
            hasText(context.getString(label)) and hasAnyAncestor(hasTestTag(DebtorsTestTags.DELETE_DIALOG)) and
