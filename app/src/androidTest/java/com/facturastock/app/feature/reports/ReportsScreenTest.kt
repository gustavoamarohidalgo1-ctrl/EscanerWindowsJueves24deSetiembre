...
class ReportsScreenTest {
    ...
    fun debtCollectionIsVisibleAtDoubleFontSizeWithoutAddingItToSalesTotals() {
        ...
        composeRule.onNodeWithText(context.getString(R.string.reports_debt_payments_total, payment.payment.amount.formatForDisplay()))
            .assertIsDisplayed()
        ...
        composeRule.onNodeWithText(context.getString(R.string.reports_debt_payment_amount, payment.payment.amount.formatForDisplay()))
            .assertIsDisplayed()
    ...
    fun refreshingPreservesTheVisibleScrollPositionAndExpandedSaleDetails() {
        ...
        val expanded = SemanticsMatcher.expectValue(
            ...
            context.getString(R.string.reports_sale_details_expanded),
        )
    ...
    fun saleSummaryExpandsAccessibleProductProfitDetailsAndCollapsesAgain() {
        ...
        composeRule.onNodeWithText(
            context.resources.getQuantityString(R.plurals.reports_line_count, 2, 2),
        ).assertIsDisplayed()
        ...
        composeRule.onNodeWithTag(toggleTag)
            ...
            .assert(
                SemanticsMatcher.expectValue(
                    SemanticsProperties.StateDescription,
                    context.getString(R.string.reports_sale_details_collapsed),
                ),
            )
            .performClick()
        ...
        composeRule.onNodeWithTag(ReportsTestTags.saleLineProfit(SALE_ID.value, FIRST_LINE_ID.value))
            .assertIsDisplayed()
            .assertTextContains(context.getString(R.string.reports_sale_line_profit))
        composeRule.onNodeWithTag(ReportsTestTags.SCREEN).performScrollToNode(hasTestTag(toggleTag))
        composeRule.onNodeWithTag(toggleTag)
            .assert(
                SemanticsMatcher.expectValue(
    ...
    fun missingHistoricalCostNeverAppearsAsZeroProfit() {
        ...
        ).assertTextContains(context.getString(R.string.reports_amount_unavailable))
        composeRule.onNodeWithTag(ReportsTestTags.SCREEN).performScrollToNode(
    ...
    fun voidConfirmationShowsDateStockWarehousesDebtAndManualRefundBeforeExplicitActions() {
        ...
        composeRule.onNodeWithText(context.getString(
            R.string.reports_void_date, NOW.formatForDisplay(RANGE.zoneId),
        )).performScrollTo().assertIsDisplayed()
        composeRule.onNodeWithText(context.getString(
            R.string.reports_void_total, preview.total.formatForDisplay(),
        )).performScrollTo().assertIsDisplayed()
        preview.lines.forEach { line ->
            composeRule.onNodeWithText(context.getString(
                R.string.reports_void_line, line.productName,
                line.quantity.value.stripTrailingZeros().toPlainString(), line.unitCode, line.locationName,
            )).performScrollTo().assertIsDisplayed()
        }
        composeRule.onNodeWithText(context.getString(
            R.string.reports_void_debt, requireNotNull(preview.debtBalanceToCancel).formatForDisplay(),
        )).performScrollTo().assertIsDisplayed()
        composeRule.onNodeWithText(context.getString(
            R.string.reports_void_refund, preview.refundAmount.formatForDisplay(),
        )).performScrollTo().assertIsDisplayed()
        assertTrue(actions.isEmpty())
        dialogButton(R.string.reports_void_cancel).assertIsEnabled().performClick()
        assertEquals(listOf(ReportsContract.Action.VoidDismissed), actions)
        dialogButton(R.string.reports_void_action).assertIsEnabled().performClick()
    ...
    fun previewLoadingDisablesConfirmationAndMutationDisablesDismissalAndPeriodChanges() {
        ...
        dialogButton(R.string.reports_void_action).assertIsNotEnabled()
        dialogButton(R.string.reports_void_cancel).assertIsEnabled()
        composeRule.runOnIdle {
            state = state.copy(isLoadingVoidPreview = false, voidPreview = voidPreview(), isVoiding = true)
        }
        dialogButton(R.string.reports_void_action).assertIsNotEnabled()
        dialogButton(R.string.reports_void_cancel).assertIsNotEnabled()
    ...
    fun sharedBusinessErrorOffersReviewAndStalePreviewExplainsWhyConfirmationIsNeededAgain() {
        ...
        composeRule.onNodeWithTag(ReportsTestTags.VOID_ERROR)
            .assertTextContains(context.getString(R.string.reports_void_error_shared))
        dialogButton(R.string.reports_void_retry).performClick()
        ...
        composeRule.onNodeWithTag(ReportsTestTags.VOID_ERROR)
            .assertTextContains(context.getString(R.string.reports_void_error_stale))
        dialogButton(R.string.reports_void_action).assertIsEnabled()
        assertEquals(1, actions.size)
    }
    ...
    fun confirmationAtDoubleFontSizeKeepsActionsVisibleAndImpactScrollable() {
        ...
        dialogButton(R.string.reports_void_action).assertIsDisplayed().assertIsEnabled()
        dialogButton(R.string.reports_void_cancel).assertIsDisplayed().assertIsEnabled()
        composeRule.onNodeWithText(context.getString(
            R.string.reports_void_refund, preview.refundAmount.formatForDisplay(),
        )).performScrollTo().assertIsDisplayed()
        dialogButton(R.string.reports_void_action).assertIsDisplayed()
        dialogButton(R.string.reports_void_cancel).assertIsDisplayed()
    }
    ...
    fun successfulVoidNoticeRemainsVisibleWithoutScrollingAndCanBeDismissed() {
        ...
        composeRule.onNodeWithText(context.getString(R.string.reports_void_notice_dismiss)).performClick()
    ...
    fun savedPdfOffersExplicitOpenAndMissingViewerKeepsSuccessVisible() {
        ...
        composeRule.onNodeWithTag(ReportsTestTags.PDF_NO_VIEWER).performScrollTo()
            .assertTextContains(context.getString(R.string.reports_pdf_no_viewer))
        composeRule.onNodeWithTag(ReportsTestTags.PDF_SUCCESS).performScrollTo().assertIsDisplayed()
    ...
    private fun dialogButton(@androidx.annotation.StringRes label: Int) = composeRule.onNode(
        hasText(context.getString(label)) and hasAnyAncestor(hasTestTag(ReportsTestTags.VOID_DIALOG)),
    )
