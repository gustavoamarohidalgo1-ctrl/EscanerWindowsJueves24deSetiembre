...
class InventoryIncrementalReadPerformanceTest {
    ...
    fun benchmarkInventorySingleProductUpdates() =
        runBlocking {
            ...
            try {
                ...
                // Sin Instrumentation: la métrica se publica en la salida estándar del test JVM.
                println("InventoryReadBenchmark: $metrics")
            } finally {
    ...
    private fun CoroutineScope.collectInventory(
        ...
        launch {
            flow.collect { destination.send(TimedInventory(it, System.nanoTime())) }
        }
