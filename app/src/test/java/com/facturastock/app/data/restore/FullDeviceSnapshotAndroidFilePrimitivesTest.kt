...
/**
 * Primitivas de archivo del restore sobre el sistema de archivos privado real del escritorio
 * (antes: `noBackupFilesDir` de Android). El directorio temporal se borra al terminar.
 */
class FullDeviceSnapshotAndroidFilePrimitivesTest {
    @get:Rule
    val tempFolder = TemporaryFolder()

    private val root by lazy { tempFolder.root.toPath().resolve("restore-test-${UUID.randomUUID()}") }
    private val coordinator = FullDeviceSnapshotSwapCoordinator()

    @Test
    fun journalCreateAndCasAreDurableOnThePrivateFilesystem() {
    ...
    @Test
    fun forwardDatabaseRenamesAreAtomicAndRecoverableOnPrivateFilesystem() {
        Files.createDirectory(root)
