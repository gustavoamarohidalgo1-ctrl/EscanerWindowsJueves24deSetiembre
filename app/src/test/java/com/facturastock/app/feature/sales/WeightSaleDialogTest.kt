...
import com.facturastock.app.resources.weight_sale_exceeds_stock
import com.facturastock.app.resources.weight_sale_missing_price
import com.facturastock.app.resources.weight_sale_price_per_kg
import com.facturastock.app.resources.weight_sale_save
import com.facturastock.app.resources.weight_sale_stale_cart
...
class WeightSaleDialogTest {
    ...
    fun amountIsDefaultAndDecimalCommaShowsExactKilosBeforeConfirming() {
        ...
        composeRule.onNodeWithText(string(Res.string.weight_sale_add)).assertIsNotEnabled()
        composeRule.onNodeWithTag(WeightSaleTestTags.INPUT).performTextInput("5,25")
        composeRule
            .onNodeWithTag(WeightSaleTestTags.PRICE)
            .assertTextContains(string(Res.string.weight_sale_price_per_kg, money("6").formatForDisplay()))
        composeRule
        ...
        composeRule
            ...
            .assertTextContains(string(Res.string.weight_sale_total, money("5.25").formatForDisplay()))
        composeRule.onNodeWithText(string(Res.string.weight_sale_add)).assertIsEnabled().performClick()
    ...
    fun kilosModeCalculatesChargeAndUsesQuantityAction() {
        ...
        composeRule
            .onNodeWithTag(WeightSaleTestTags.TOTAL, useUnmergedTree = true)
            .assertTextContains(string(Res.string.weight_sale_total, money("7.50").formatForDisplay()))
        composeRule.onNodeWithText(string(Res.string.weight_sale_add)).assertIsEnabled().performClick()
    ...
    fun invalidOrOverStockAmountCannotConfirmButExactAvailableStockCan() {
        ...
        composeRule
            ...
            .assertTextContains(string(Res.string.weight_sale_exceeds_stock))

        composeRule.runOnIdle { state.value = state.value.copy(amountInput = "12") }
        composeRule.onNodeWithText(string(Res.string.weight_sale_add)).assertIsEnabled()
    ...
    fun unavailablePriceAndStaleCartExplainWhyConfirmationIsBlocked() {
        ...
        composeRule
            .onNodeWithTag(WeightSaleTestTags.ERROR)
            .assertTextContains(string(Res.string.weight_sale_missing_price))

        composeRule.runOnIdle { state.value = initial.copy(failure = WeightSaleFailure.STALE_CART) }
        composeRule.onNodeWithText(string(Res.string.weight_sale_add)).assertIsNotEnabled()
        composeRule
            .onNodeWithTag(WeightSaleTestTags.ERROR)
            .assertTextContains(string(Res.string.weight_sale_stale_cart))
    }
    ...
    fun savingBlocksBothModesInputConfirmationAndDismissal() {
        ...
        composeRule.onNodeWithText(string(Res.string.weight_sale_save)).assertIsNotEnabled()
        composeRule.onNodeWithText(string(Res.string.action_cancel)).assertIsNotEnabled()
    }
    ...
    fun periodicKilosHaveAnApproximationMarkWhileTheAmountRemainsExact() {
        ...
        composeRule
            .onNodeWithTag(WeightSaleTestTags.TOTAL, useUnmergedTree = true)
            .assertTextContains(string(Res.string.weight_sale_total, money("1").formatForDisplay()))
        composeRule.onNodeWithText(string(Res.string.weight_sale_add)).assertIsEnabled()
    }
    ...
    fun salesScreenHostsWeightEditorForCreditSaleAndDismissesThroughItsAction() {
        ...
        composeRule.onNodeWithText(string(Res.string.action_cancel)).performClick()
    ...
    private fun string(
        id: StringResource,
        vararg args: Any,
    ): String = runBlocking { getString(id, *args) }
