...
class OcrScreenTest {
    ...
    fun realStagesAreAnnouncedAsIndeterminateProgressWithoutPercentages() {
        ...
        listOf(
            InvoiceOcrStage.PREPARING to Res.string.ocr_stage_preparing,
            InvoiceOcrStage.READING to Res.string.ocr_stage_reading,
            InvoiceOcrStage.MERGING_PAGES to Res.string.ocr_stage_merging_pages,
    ...
    fun productImportFailuresExplainTheOutcomeAndOfferRetry() {
        ...
        composeRule.onNodeWithText(str(Res.string.ocr_no_products_title))
            .assertIsDisplayed()
        ...
        composeRule.onNodeWithText(str(Res.string.ocr_product_save_error_title))
            .assertIsDisplayed()
        composeRule.onNodeWithTag(OcrTestTags.RETRY).assertIsEnabled()
    }
    ...
    fun savingProductsKeepsProgressVisibleWithoutNavigationActions() {
        ...
        val saving = str(Res.string.ocr_saving_products)
    ...
    fun interruptedRunExplainsLocalPersistenceAndOffersSingleRecoveryAction() {
        ...
        composeRule.onNodeWithText(str(Res.string.ocr_interrupted_status))
            .assertIsDisplayed()
        composeRule.onNodeWithText(str(Res.string.ocr_interrupted_message))
            .assertIsDisplayed()
        composeRule.onNodeWithTag(OcrTestTags.ENTER_MANUALLY).assertDoesNotExist()
        composeRule.onNodeWithTag(OcrTestTags.RETRY).assertIsEnabled().performClick()
        assertEquals(OcrContract.Action.Retry, action)
        ...
        composeRule.onNodeWithText(str(Res.string.ocr_recovering)).assertIsDisplayed()
    }
    ...
    fun cancellingKeepsIndeterminateFeedbackAndDisablesTheAction() {
        ...
        val cancelling = str(Res.string.ocr_cancelling)
