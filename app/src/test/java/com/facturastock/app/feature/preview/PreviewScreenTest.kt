...
class PreviewScreenTest {
    ...
    fun actionsIndicatorAndProcessButtonAreVisible() {
        ...
        composeRule.onNodeWithText(
            str(Res.string.preview_page_indicator, 1, 2),
        ).assertIsDisplayed()
    ...
    fun failedPageEditExplainsThatTheOriginalPageWasPreserved() {
        ...
        composeRule.onNodeWithText(str(Res.string.preview_edit_error_title))
            .assertIsDisplayed()
        composeRule.onNodeWithText(str(Res.string.preview_edit_error_message))
            .assertIsDisplayed()
    ...
    fun deleteAsksForConfirmationBeforeRemoving() {
        ...
        composeRule.onNodeWithText(
            str(Res.string.preview_delete_title),
        ).assertIsDisplayed()
        composeRule.onNode(
            hasText(str(Res.string.preview_delete_confirm)) and
    ...
    /**
     * Control del mecanismo que usa [qualityWarningExplainsBlurAndExposureAndOffersContinueOrRetake]:
     * el Atrás del sistema (Esc en la ventana) llega al diálogo y lo cierra cuando éste lo permite.
     */
    @Test
    fun systemBackDismissesTheDeleteConfirmation() {
        val received = mutableListOf<PreviewContract.Action>()
        setContentWithSystemBack {
            FacturaStockTheme {
                PreviewScreen(
                    state = fakeState(showDeleteConfirm = true),
                    onAction = { received += it },
        ...
        assertEquals(listOf(PreviewContract.Action.DeleteDismissed), received)
    }
    ...
    fun combinedErrorsDoNotCollapseTheImageOrHideRecoveryInACompactWindow() {
        ...
        composeRule.onNodeWithTag(PreviewTestTags.PAGE_IMAGE).assertIsDisplayed()
        composeRule.onNodeWithText(str(Res.string.preview_process_error_title))
            .performScrollTo()
            .assertIsDisplayed()
        composeRule.onNodeWithText(str(Res.string.preview_edit_error_title))
            .performScrollTo()
    ...
    fun cropHandlesExposeDistinctTalkBackLabelsPositionsAndActions() {
        ...
        val cornerDescriptions = mapOf(
            PreviewContract.CropCorner.TOP_LEFT to Res.string.preview_crop_handle_top_left,
            PreviewContract.CropCorner.TOP_RIGHT to Res.string.preview_crop_handle_top_right,
            PreviewContract.CropCorner.BOTTOM_LEFT to Res.string.preview_crop_handle_bottom_left,
            PreviewContract.CropCorner.BOTTOM_RIGHT to Res.string.preview_crop_handle_bottom_right,
        )
        assertEquals(4, cornerDescriptions.values.map { str(it) }.toSet().size)
        cornerDescriptions.forEach { (corner, descriptionRes) ->
            composeRule.onNodeWithTag(PreviewTestTags.cropHandle(corner))
                .assert(
                    SemanticsMatcher.expectValue(
                        SemanticsProperties.ContentDescription,
                        listOf(str(descriptionRes)),
                    ),
                )
        }
        ...
        val topLeft = composeRule.onNodeWithTag(
            ...
            .assert(
                SemanticsMatcher.expectValue(
                    ...
                    str(Res.string.preview_crop_handle_position, 10, 10),
                ),
            )
        ...
        assertEquals(
            listOf(
                Res.string.preview_crop_move_left,
                Res.string.preview_crop_move_right,
                Res.string.preview_crop_move_up,
                Res.string.preview_crop_move_down,
            ).map { str(it) },
            actions.map { it.label },
        )
        assertTrue(
            actions.single {
                it.label == str(Res.string.preview_crop_move_right)
            }.action(),
        )
        assertEquals(
            listOf(
    ...
    fun qualityWarningExplainsBlurAndExposureAndOffersContinueOrRetake() {
        ...
        composeRule.onNodeWithText(
            str(Res.string.preview_quality_warning_title),
        ).assertIsDisplayed()
        val detail = str(Res.string.preview_quality_underexposed,
            42,
            "68.0%",
        )
        composeRule.onNodeWithText(
            str(Res.string.preview_quality_warning_page, 1, detail),
            substring = true,
        ).assertIsDisplayed()
        val blurDetail = str(Res.string.preview_quality_blur,
            500,
            1_200,
        )
        composeRule.onNodeWithText(
            str(Res.string.preview_quality_warning_page, 1, blurDetail),
            substring = true,
        ).assertIsDisplayed()
        composeRule.onNodeWithText(
            str(Res.string.preview_quality_continue),
        ).assertIsDisplayed()
        composeRule.onNodeWithText(
            str(Res.string.preview_quality_retake),
        ).assertIsDisplayed()
    ...
    private val systemBack = SceneSystemBack()

    private fun setContentWithSystemBack(content: @Composable () -> Unit) {
        composeRule.setContent {
            systemBack.capture()
            content()
        }
    ...
    private fun pressSystemBack() = systemBack.pressBack(composeRule)
