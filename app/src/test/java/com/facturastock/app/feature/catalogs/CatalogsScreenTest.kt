...
class CatalogsScreenTest {
    ...
    @Test
    fun specialProductFormHasKilosCostAndPriceWithResolvedWarehouseAndNoBarcodeInput() {
        ...
        composeRule.onNodeWithText(str(Res.string.catalog_special_product_title)).assertIsDisplayed()
        composeRule.onNodeWithTag(CatalogsTestTags.PRODUCT_BARCODE).assertDoesNotExist()
        composeRule.onNodeWithText(str(Res.string.catalog_special_product_quantity)).performScrollTo().assertIsDisplayed()
        composeRule.onNodeWithTag(CatalogsTestTags.PRODUCT_PURCHASE_PRICE).performScrollTo().assertTextContains("0")
        // Desde cd3d3e3 el alta por kilo no muestra selector de unidad ni de almacén: usa el primer
        // almacén activo ya resuelto, así que no hay aviso de almacén pendiente.
        composeRule.onNodeWithText("Mostrador").assertDoesNotExist()
        composeRule.onNodeWithText(str(Res.string.catalog_special_product_location_required)).assertDoesNotExist()
    ...
    fun tabsSearchAndAddAreDiscoverableAndEmitActions() {
        ...
        composeRule.onNodeWithText(str(Res.string.home_delete_dialog_title))
            .assertDoesNotExist()
    }
    ...
    fun productsHideStatusFiltersBadgesAndEvenLegacyPendingStatusDialogs() {
        ...
        composeRule.onNodeWithText(str(Res.string.catalog_filter_active)).assertDoesNotExist()
        composeRule.onNodeWithText(str(Res.string.catalog_filter_archived)).assertDoesNotExist()
        composeRule.onNodeWithText(str(Res.string.catalog_filter_all)).assertDoesNotExist()
        ...
        composeRule.onNodeWithText(str(Res.string.catalog_action_archive)).assertDoesNotExist()
        composeRule.onNodeWithText(str(Res.string.catalog_action_restore)).assertDoesNotExist()
        composeRule.onNodeWithText(str(Res.string.catalog_action_edit)).assertIsDisplayed()
    }
    ...
    fun unitCatalogRetainsStatusFiltersAndArchiveAction() {
        ...
        composeRule.onNodeWithText(str(Res.string.catalog_filter_active)).assertIsDisplayed()
        composeRule.onNodeWithText(str(Res.string.catalog_filter_archived)).assertIsDisplayed()
        composeRule.onNodeWithText(str(Res.string.catalog_filter_all)).assertIsDisplayed()
        ...
        composeRule.onNodeWithText(str(Res.string.catalog_action_archive)).assertIsDisplayed()
    }
    ...
    fun supplierFormExplainsThatRucValidationIsLocalAndOffersNoDelete() {
        ...
        composeRule.onNodeWithText(str(Res.string.catalog_ruc_local_notice))
            .assertIsDisplayed()
        composeRule.onNodeWithText(str(Res.string.action_save)).assertIsDisplayed()
        composeRule.onNodeWithText(str(Res.string.catalog_action_archive))
            .assertDoesNotExist()
    }
    ...
    fun supplierUnitAndLocationFieldsStayDisabledUntilSavingFinishes() {
        ...
        val forms = listOf(
            Form.SupplierForm(title = "Proveedor local", tradeName = "Mi proveedor", ruc = "20131312955") to listOf(
                Res.string.catalog_legal_name_label,
                Res.string.catalog_trade_name_label,
                Res.string.catalog_ruc_label,
            ),
            Form.UnitForm(title = "Unidad", code = "NIU", symbol = "u") to listOf(
                Res.string.catalog_name_label,
                Res.string.catalog_unit_code_label,
                Res.string.catalog_unit_symbol_label,
            ),
            Form.LocationForm(title = "Almacén") to listOf(Res.string.catalog_name_label),
        )
        forms.forEach { (form, labels) ->
            composeRule.runOnIdle { state = state.copy(form = form, isSaving = true) }
            labels.forEach { label ->
                composeRule.onNodeWithText(str(label)).performScrollTo().assertIsNotEnabled()
            }
            ...
            labels.forEach { label ->
                composeRule.onNodeWithText(str(label)).performScrollTo().assertIsEnabled()
            }
        }
        assertTrue(actions.isEmpty())
        composeRule.onNodeWithText(str(Res.string.catalog_name_label)).performTextReplacement("Sucursal")
    ...
    fun inventorySalePriceExplainsZeroAndOverflowWithoutChangingEnteredValues() {
        ...
        val error = str(Res.string.catalog_polish_sale_price_error, "90071992547409.91", "PEN", 2)
    ...
    fun clearingKnownPurchasePriceExplainsRequiredValueWithoutTreatingUnknownCostAsAnError() {
        ...
        val requiredError = str(Res.string.catalog_polish_purchase_price_required)
        val unknownHelp = str(Res.string.inventory_stock_editor_missing_cost_help)
    ...
    fun supplierFormExposesPaneTitleAndCapsWidthInWideWindow() {
        val paneTitle = str(Res.string.catalog_create_supplier)
        inSimulatedWindow(width = 840.dp, content = {
            FacturaStockTheme {
                CatalogsScreen(
                    state = State(
                        section = Section.SUPPLIERS,
            ...
            onNodeWithTag(CatalogsTestTags.FORM)
                .assertIsDisplayed()
                .assert(
    ...
    fun productFormActionsRemainReachableInCompactWindowAtTwoHundredPercentFontScale() {
        val actions = mutableListOf<Action>()
        inSimulatedWindow(width = 360.dp, fontScale = 2f, content = {
            FacturaStockTheme {
                CatalogsScreen(
                    state = State(
            ...
            val form = onNodeWithTag(CatalogsTestTags.FORM).assertIsDisplayed()
            ...
            onNodeWithText(str(Res.string.action_save))
                .performScrollTo()
                .assertIsDisplayed()
                .assertHasClickAction()
                .performClick()
            onNodeWithText(str(Res.string.action_cancel))
                .performScrollTo()
                .assertIsDisplayed()
                .assertHasClickAction()
                .performClick()
    ...
    fun inventoryEditorPrefillsThreeExplicitValuesAndKeepsSaveVisibleWhileScrolling() {
        ...
        inSimulatedWindow(width = 840.dp, content = {
            ...
            onNodeWithTag(CatalogsTestTags.PRODUCT_QUANTITY)
                .performScrollTo().assertIsDisplayed().assertTextContains("10").performTextReplacement("7")
            onNodeWithTag(CatalogsTestTags.PRODUCT_PURCHASE_PRICE)
            ...
            onNodeWithTag(CatalogsTestTags.PRODUCT_BARCODE).performScrollTo().assertIsDisplayed()
            onNodeWithTag(CatalogsTestTags.SAVE_FORM).assertIsDisplayed().assertIsEnabled().performClick()
            onNodeWithText(str(Res.string.action_cancel)).assertIsDisplayed()
        }
        assertTrue(
            actions.containsAll(
                listOf(
    ...
    fun productDetailShowsUnitStockAverageCostAndSupplierAlias() {
        ...
        composeRule.onNodeWithText(
            str(Res.string.catalog_product_unit_value, "NIU"),
        ).assertIsDisplayed()
        composeRule.onNodeWithText(
            str(Res.string.catalog_product_total_stock, "25", "NIU"),
        ).assertIsDisplayed()
        composeRule.onNodeWithText(
            str(Res.string.catalog_product_average_cost, "4.50 PEN"),
    ...
    fun negativeStockKeepsPositionVisibleAndExplainsUnavailableAverageCost() {
        ...
        composeRule.onNodeWithText(str(Res.string.catalog_product_total_stock, "-2", "NIU"))
            .performScrollTo().assertIsDisplayed()
        composeRule.onNodeWithText(str(Res.string.catalog_polish_average_cost_unavailable, "PEN"))
            .performScrollTo().assertIsDisplayed()
        composeRule.onNodeWithText(str(Res.string.catalog_product_position_single, "-2", "4.50 PEN"))
            .performScrollTo().assertIsDisplayed()
    }
    ...
    /**
     * Sustituye a la ventana simulada de Android (que cambiaba la densidad de la vista anfitriona):
     * aquí la escena de prueba tiene de verdad el ancho pedido, que es lo que mide un diálogo de
     * escritorio, y [fontScale] se aplica sobre la densidad de la escena.
     */
    private fun inSimulatedWindow(
        width: Dp,
        fontScale: Float = 1f,
        content: @Composable () -> Unit,
        test: ComposeUiTest.() -> Unit,
    ) = runDesktopComposeUiTest(width = width.value.roundToInt(), height = SIMULATED_WINDOW_HEIGHT) {
        check(density.density == 1f) { "La conversión dp→px de la ventana simulada asume densidad 1" }
        setContent {
    ...
    private companion object {
        const val SIMULATED_WINDOW_HEIGHT = 780
