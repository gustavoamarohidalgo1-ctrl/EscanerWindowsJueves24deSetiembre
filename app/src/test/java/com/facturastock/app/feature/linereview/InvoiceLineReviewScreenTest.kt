...
class InvoiceLineReviewScreenTest {
    ...
    fun oneHundredStableCardsScrollLazilyWhileTheExactSummaryStaysFixed() {
        ...
        composeRule.onNodeWithText(
            str(Res.string.line_review_limit_reached),
        ).assertIsDisplayed()
        ...
        composeRule.onNodeWithTag(InvoiceLineReviewTestTags.card(last.lineId))
            .assert(
                SemanticsMatcher("keeps metrics after lazy reuse") { node ->
                    ...
                    "${str(Res.string.line_review_metric_cost)}: S/ 10.00" in spoken &&
                        "${str(Res.string.line_review_metric_total)}: S/ 11.80" in spoken
                },
            )
    ...
    fun reorderMenuAtTwoHundredPercentKeepsTouchTargetsTagsAndTalkBackCopy() {
        ...
        val lineNumber = str(Res.string.line_review_line_number, 2)
        composeRule.onNodeWithTag(InvoiceLineReviewTestTags.LIST)
        ...
        composeRule.onNodeWithContentDescription(
            str(
                Res.string.line_review_move_up_accessibility,
                lineNumber,
                "Producto 2",
            ),
        ).assertIsDisplayed()
        ...
        composeRule.onNodeWithTag(InvoiceLineReviewTestTags.PENDING_FILTER)
            .assert(
                SemanticsMatcher.expectValue(
                    ...
                    str(Res.string.line_review_total_count, 2),
                ),
            )
            .performClick()
            .assert(
                SemanticsMatcher.expectValue(
                    SemanticsProperties.StateDescription,
                    str(Res.string.line_review_pending_count, 1, 2),
                ),
            )
        composeRule
    ...
    fun deletionRequiresConfirmationAndTheSameStableLineCanBeRestored() {
        ...
        composeRule.onNodeWithText(
            str(Res.string.line_review_delete_confirm),
        ).performClick()
    ...
    fun editorExposesEveryFieldAndSeparatesOcrCalculatedAndWrittenValues() {
        ...
        composeRule
            ...
            .assert(
                SemanticsMatcher.expectValue(
                    ...
                    listOf(
                        str(Res.string.line_review_summary_sum_label) +
                            ": S/ 11.83. " +
                            str(Res.string.line_review_summary_difference_label) +
                            ": +S/ 0.03. " +
                            str(
                                Res.string.line_review_summary_differs,
                                "+S/ 0.03",
                            ),
                    ),
                ),
        ...
        val ocrReference = str(Res.string.line_review_ocr_reference, "11.80")
        val calculatedReference = str(
        ...
        composeRule.onNodeWithText(
            str(Res.string.line_review_source_written),
        ).assertIsDisplayed()
    ...
    fun incompleteSummaryNeverAnnouncesThatAmountsMatch() {
        ...
        composeRule.onNodeWithText(
            str(Res.string.line_review_summary_incomplete),
        ).assertIsDisplayed()
        composeRule.onNodeWithText(
            str(Res.string.line_review_summary_matches),
        ).assertDoesNotExist()
    }
    ...
    fun exemptLineShowsTaxMeaningInsteadOfCurrencyPending() {
        ...
        composeRule.onNodeWithText(
            str(Res.string.line_review_tax_exempt_value),
            substring = true,
        ).assertIsDisplayed()
    ...
    fun compactMetricTextNodesKeepVisibleValuesAndCompleteAccessibleDescription() {
        ...
        composeRule.onNodeWithText(
            str(
                Res.string.line_review_metric_value,
                str(Res.string.line_review_metric_cost),
                "S/ 10.00",
            ),
        ).assertIsDisplayed()
        composeRule.onNodeWithText(
            str(
                Res.string.line_review_metric_value,
                str(Res.string.line_review_metric_igv),
                "S/ 1.80",
            ),
        ).assertIsDisplayed()
        composeRule.onNodeWithText(
            str(
                Res.string.line_review_metric_value,
                str(Res.string.line_review_metric_total),
                "S/ 11.80",
            ),
        ).assertIsDisplayed()
    }
    ...
    fun repeatedLineActionsExposeTheLineAndProductToTalkBack() {
        ...
        val lineNumber = str(Res.string.line_review_line_number, 1)
        composeRule.onNodeWithContentDescription(
            str(
                Res.string.line_review_edit_accessibility,
                lineNumber,
                "Café molido",
            ),
        ...
        composeRule.onNodeWithContentDescription(
            str(
                Res.string.line_review_delete_accessibility,
                lineNumber,
                "Café molido",
            ),
        ).performScrollTo().assertIsDisplayed()
    ...
    /**
     * Las tarjetas se preparan en `Dispatchers.Default` (produceState) y mientras tanto se muestra
     * un marcador "Cargando…". La sincronización de Compose no espera ese despachador, así que se
     * espera a que ningún marcador siga visible.
     */
    private fun waitForCardPresentations() {
        val loading = str(Res.string.line_review_loading)
        composeRule.waitUntil(timeoutMillis = 5_000L) {
            composeRule.onAllNodes(hasContentDescription(loading))
                .fetchSemanticsNodes()
                .isEmpty()
        }
    }
