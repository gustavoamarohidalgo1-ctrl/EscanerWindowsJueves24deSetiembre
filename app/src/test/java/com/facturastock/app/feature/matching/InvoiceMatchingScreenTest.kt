...
class InvoiceMatchingScreenTest {
    ...
    fun typingDecimalCommasPreservesPriceAndQuantityInCallbacksAndFields() {
        ...
        val price = composeRule.onNodeWithText(text(Res.string.catalog_product_price_label))
        listOf("12", ",", "50").forEach { price.performTextInput(it) }
        price.assertTextContains("12,50")

        val quantity = composeRule.onNodeWithText(text(Res.string.catalog_product_quantity_label))
    ...
    fun failedProductSearchOffersRetryThenShowsLoadingAndAnEmptyResultSeparately() {
        ...
        composeRule.onNodeWithText(text(Res.string.matching_search_error)).assertIsDisplayed()
        composeRule.onNodeWithContentDescription(text(Res.string.matching_search_loading)).assertDoesNotExist()
        composeRule.onNodeWithText(text(Res.string.matching_dialog_link_no_results)).assertDoesNotExist()
        composeRule.onNodeWithText(text(Res.string.action_retry)).assertIsEnabled().performClick()

        composeRule.onNodeWithContentDescription(text(Res.string.matching_search_loading)).assertIsDisplayed()
        composeRule.onNodeWithText(text(Res.string.matching_search_error)).assertDoesNotExist()
        composeRule.onNodeWithText(text(Res.string.action_retry)).assertDoesNotExist()
        composeRule.onNodeWithText(text(Res.string.matching_dialog_link_no_results)).assertDoesNotExist()
        composeRule.runOnIdle {
        ...
        composeRule.onNodeWithText(text(Res.string.matching_dialog_link_no_results)).assertIsDisplayed()
        composeRule.onNodeWithContentDescription(text(Res.string.matching_search_loading)).assertDoesNotExist()
        composeRule.onNodeWithText(text(Res.string.matching_search_error)).assertDoesNotExist()
        composeRule.onNodeWithText(text(Res.string.action_retry)).assertDoesNotExist()
    }
    ...
    fun savingDisablesProductFieldsAndDialogButtonsUntilTheOperationFinishes() {
        ...
        val labels =
            listOf(
                Res.string.catalog_barcode_label,
                Res.string.catalog_name_label,
                Res.string.catalog_product_price_label,
                Res.string.catalog_product_quantity_label,
        ...
        composeRule.onNodeWithText(text(Res.string.catalog_product_price_label)).assertTextContains("12,50")
        composeRule.onNodeWithText(text(Res.string.catalog_product_quantity_label)).assertTextContains("1,5")
    }

    private fun text(resource: StringResource): String = str(resource)
}
